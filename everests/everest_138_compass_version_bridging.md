# Everest 138 | ZKAC Bridging Across Compass Versions

**Canonical vault path:** [`ZKAC_COMPASS_VERSION_BRIDGE_v0.md`](../ZKAC_COMPASS_VERSION_BRIDGE_v0.md) (normative; this shard mirrors it for Everest indexing).
...
This Everest specifies **Compass primitive bridging only**. CredexAI platform bridging is Everest 135. Pact and Witness wire bumps are Everests 136 and 137. Shared N/N-1 discipline lives in [`ZKAC_VERSION_BRIDGING_v0.md`](../ZKAC_VERSION_BRIDGING_v0.md) §0 and §4.
...
[`CALM_COMPASS_SCOPE_STATEMENT.md`](../CALM_COMPASS_SCOPE_STATEMENT.md) and predicates §4 refusal floor: items may be added, never removed. Compass wire v1 MUST NOT weaken prohibited uses or enable employment, insurance, lending, or medical screening via new optional fields.

---

## §8 | Envelope downgrade rules (Compass section)

### §8.1 | Allowed transforms (v1 to v0)
...
### §9.4 | Top-level wire check

Compass bridging sits inside composite envelopes. Verifiers MUST call `bridge_version_accepted(envelope.wire_version, supported)` from [`version_bridge.py`](../../CredexAI/calm_witness/version_bridge.py) before Compass-specific parsing.

---

## §10 | Reference implementation and gates

| Artifact | Path |
| --- | --- |
| Shared bridging (135 to 138) | [`ZKAC_VERSION_BRIDGING_v0.md`](../ZKAC_VERSION_BRIDGING_v0.md) |
| This Everest (138 only) | [`everest_138_compass_version_bridging.md`](everest_138_compass_version_bridging.md) |
| `bridge_version_accepted` | [`~/CredexAI/calm_witness/version_bridge.py`](../../CredexAI/calm_witness/version_bridge.py) |
| Everest 138 gate | [`~/CredexAI/scripts/everest_138_zkac_compass_version_bridging_gate.py`](../../CredexAI/scripts/everest_138_zkac_compass_version_bridging_gate.py) |
...
6. `bridge_version_accepted` passes for supported wire sets; golden corpus green.

---

**One-line result:** Everest 138 BAGGED: Compass `cwp.compass.v0.*` IDs and counter-claim semantics stay stable across wire and taxonomy bumps; minor evidence fields only; ≤5 predicates per envelope; `bridge_version_accepted` gates the shared N/N-1 window.
