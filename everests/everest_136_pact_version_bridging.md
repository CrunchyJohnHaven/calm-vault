# Everest 136 | ZKAC Bridging Across Pact Versions

**Closes Everest 136 of [`ZKAC_NEXT_200_EVERESTS.md`](../ZKAC_NEXT_200_EVERESTS.md).**

**Canonical spec:** [`ZKAC_PACT_VERSION_BRIDGE_v0.md`](../ZKAC_PACT_VERSION_BRIDGE_v0.md) (normative). This file is the Everest narrative companion.
...
**Acceptance test:** A composite envelope minted under Pact wire `calm-pact/v1` verifies on counterparties that advertise only `calm-pact/v0-stub` support during the published deprecation window, and the inverse holds for v0-stub minters against v1 verifiers. Unknown `pact_wire` values reject with `unsupported_pact_version`, never with a false `equality_bit`. Bridging rules are normative for `pact_digest` versioning, bit semantics, scope forfeits, and anti-purity-test output shape.

---

## §1 | Problem statement

Calm Pact proves categorical directive equality between two autonomous agents. In composite ZKAC envelopes (Everest 122), the Pact section carries a session binding digest, a single disclosed bit, and proof bytes. When the Pact wire bumps from `calm-pact/v0-stub` (placeholder Σ-proof) to `calm-pact/v1` (production Ristretto255 equality proof per [`CALM_PACT_PROTOCOL_v0.md`](../CALM_PACT_PROTOCOL_v0.md) §4), counterparties must not fracture.
...
This Everest specifies **Pact primitive bridging only**. CredexAI platform bridging is Everest 135. Witness and Compass wire bumps are Everests 137 and 138. Shared window discipline lives in [`ZKAC_VERSION_BRIDGING_v0.md`](../ZKAC_VERSION_BRIDGING_v0.md).
...
Reference implementation (`pact_session_digest` in [`zkac_envelope.py`](../../CredexAI/calm_witness/zkac_envelope.py)) defines v0-stub material:
...
Reference checker: `bridge_pact_accepted(pact_wire, frozenset(supported_pact_wires))` in [`version_bridge.py`](../../CredexAI/calm_witness/version_bridge.py).
...
### §6.2 | Wire bump is not new capability
...
### §6.3 | License visibility

`pact_wire` and operator attestation fingerprint appear in envelope metadata and well-known manifests. Misuse after a bump is attributable to the deploying Operator.
...
| Artifact | Path |
| --- | --- |
| Canonical spec | [`ZKAC_PACT_VERSION_BRIDGE_v0.md`](../ZKAC_PACT_VERSION_BRIDGE_v0.md) |
| Shared bridging rules (135 to 138) | [`ZKAC_VERSION_BRIDGING_v0.md`](../ZKAC_VERSION_BRIDGING_v0.md) §2 |
| This Everest (136 narrative) | [`everest_136_pact_version_bridging.md`](everest_136_pact_version_bridging.md) |
| `pact_session_digest` | [`~/CredexAI/calm_witness/zkac_envelope.py`](../../CredexAI/calm_witness/zkac_envelope.py) |
| `bridge_pact_accepted` | [`~/CredexAI/calm_witness/version_bridge.py`](../../CredexAI/calm_witness/version_bridge.py) |
| Everest 136 gate | [`~/CredexAI/scripts/everest_136_zkac_pact_version_bridge_gate.py`](../../CredexAI/scripts/everest_136_zkac_pact_version_bridge_gate.py) |
...
5. Scope forfeits and anti-purity-test rules documented; no similarity score outputs.
